...
async function generateTransactionPDF(data, user) {
...
async function generateBulkPDF(data, user) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument();
    let buffers = [];
    ...
    doc
      .rect(
        doc.x,
        doc.y,
        doc.widthOfString("Bulk Transaction Receipt"),
    ...
    doc
      .fillColor("black")
      .fontSize(18)
      .text("Bulk Transaction Receipt", { align: "center" });
    doc.moveDown();
    ...
    doc.fontSize(12).text("Transactions:", { underline: true });
    let totalAmountComplete = 0;
    let totalAmountFailed = 0;
    data.forEach((txn, index) => {
      if (doc.y > 700) doc.addPage(); // Add new page if needed
      doc.moveDown(0.5);
      doc.text(
        `${index + 1}. Date: ${new Date(txn.createdAt).toLocaleString()}`
      );
      doc.text(`   Amount: KES ${txn.amount}`);
      doc.text(`   Status: ${txn.status}`);
      if (txn.status === "COMPLETED") {
        totalAmountComplete += parseInt(txn.amount);
      }
      if (txn.status === "FAILED") {
        totalAmountFailed += parseInt(txn.amount);
      }
    });

    doc.moveDown();
    doc.fontSize(14).text(`Total Amount Complete: KES ${totalAmountComplete}`, {
      bold: true,
    });
    doc.moveDown();
    doc
      .fontSize(14)
      .text(`Total Amount Failed: KES ${totalAmountFailed}`, { bold: true });
    doc.text("Thank you for your transaction!", { align: "center" });
    doc.end();
  });
}
